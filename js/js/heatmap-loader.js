(function(){
  ...
  function updateExecutiveKPIs(){
    ...
    const critical = summary.filter(r => r && r.estado === 'Crítica').length;
    const total = summary.length || 1;
    const optimal = summary.filter(r => r && r.estado === 'Óptima').length;
    const coverage = Math.round((optimal / total) * 100);
    const units = routes.length;
